...
const checkDataExists = async (tableName) => {
    const result = await db.query(`SELECT EXISTS (SELECT 1 FROM ${tableName} LIMIT 1);`);
    return result.rows[0].exists;
};
...
const createTables = async () => {
    ...
    const recomendaciones = `
        CREATE TABLE recomendaciones (
        id SERIAL PRIMARY KEY,
        nombre_interpretador VARCHAR(100) NOT NULL,
        titulo VARCHAR(100) NOT NULL
        );
    `;
    const insertRecomendaciones = `
    ...
    try {
        if (!(await checkTableExists('roles'))) {
            await db.query(roles);
            console.log('Table "roles" created successfully.');
        }
        if (!(await checkDataExists('roles'))) {
            await db.query(insertRoles);
        ...
        if (!(await checkTableExists('bebe'))) {
            await db.query(bebe);
            console.log('Table "bebe" created successfully.');
        }

        if (!(await checkTableExists('vacunas'))) {
            await db.query(vacunas);
            console.log('Table "vacunas" created successfully.');
        }
        if (!(await checkDataExists('vacunas'))) {
            await db.query(insertVacunas);
            console.log('Initial vacunas inserted successfully.');
        }

        if (!(await checkTableExists('esquema_vacunacion'))) {
            await db.query(esquema_vacunacion);
            console.log('Table "esquema_vacunacion" created successfully.');
        }

        if (!(await checkTableExists('recordatorio_medicamento'))) {
            await db.query(recordatorio_medicamento);
            console.log('Table "recordatorio_medicamento" created successfully.');
        }

        if (!(await checkTableExists('recomendaciones'))) {
            await db.query(recomendaciones);
            console.log('Table "recomendaciones" created successfully.');
        }
        if (!(await checkDataExists('recomendaciones'))) {
            await db.query(insertRecomendaciones);
            console.log('Initial recomendaciones inserted successfully.');
        }

        if (!(await checkTableExists('soluciones'))) {
            await db.query(soluciones);
            console.log('Table "soluciones" created successfully.');
        }
        if (!(await checkDataExists('soluciones'))) {
            await db.query(insertSoluciones);
            console.log('Initial soluciones inserted successfully.');
        }

        if (!(await checkTableExists('datos'))) {
            await db.query(datos);
            console.log('Table "datos" created successfully.');
        }
        if (!(await checkTableExists('chat'))) {
            await db.query(chat);
            console.log('Table "chat" created successfully.');
        }
